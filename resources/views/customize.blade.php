...
<div id="price" hidden>Price: {{$product->price}} Php</div>
<div id="product_id" hidden>{{$product->id}}</div>
<div id="finalPrice" class="mt-6 font-bold text-lg text-blue-600">Price: {{$product->price}} Php</div>
...
<div class="product-buttons mt-4">
    <button type="button" class="btn btn-solid hover-solid btn-animation" onclick="saveCanvasToDatabase()">
...
function updateTotalCharge() {
    // Get the current product price from the DOM
    const productPriceText = $('#price').text(); // Get the text content of the price div
    const productPrice = parseFloat(productPriceText.replace(/[^\d.-]/g, '')); // Convert to a float

    // Calculate the total charge
    const totalCharge = totalTextCharge + totalImageCharge;

    // Calculate the final price
    const finalPrice = productPrice + totalCharge; // Correctly adds the charges to the product price

    // Update the total charge display
    $('#totalCharge').text(`Total Charge: ₱${totalCharge}`); // Update additional charges
    $('#finalPrice').text(`Price: ${finalPrice.toFixed(2)}`); // Display the final price
}
...
$(document).ready(function() {
    ...
    $('#downloadBtn').click(saveCanvasToDatabase);
...
// Function to download the #container content (image + canvas) as a PNG
    ...
    fetch('/check-existing-cart-item', { // Adjust the URL to your route for checking existing cart items
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
        },
        body: JSON.stringify({ product_id: productId }) // Send the product ID for checking
    })
    .then(response => response.json())
    .then(data => {
        if (data.exists) {
            // Show SweetAlert if the product already exists in the cart
            Swal.fire({
                title: 'Warning!',
                text: 'You already have a customized product in your cart. Do you want to continue?',
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    // If the user confirms, proceed to save the new design
                    saveDesign(container, finalPrice, quantity, productId, productTitle);
                }
            });
        } else {
            // No existing item, proceed to save immediately
            saveDesign(container, finalPrice, quantity, productId, productTitle);
        }
    })
    .catch(error => {
        console.error('Error checking existing cart item:', error);
        toastr.error('An error occurred while checking your cart.');
    });
}
